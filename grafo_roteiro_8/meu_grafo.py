...
class MeuGrafo(GrafoListaAdjacencia):
    ...
    def kruskal(self):
        ...
        while len(arvore.A) < len(self.N) - 1:
            if len(E_buckets[j]) == 0:
                ...
                if find(v1, pai) != find(v2, pai):
                    if not arvore.existeVertice(v1): arvore.adicionaVertice(v1)
                    if not arvore.existeVertice(v2): arvore.adicionaVertice(v2)
                    arvore.adicionaAresta(rotuloAresta, v1, v2, pesoAresta)
    ...
    def prim(self):
        ...
        while len(vertices_mst) != len(self.N):
            ...
            arvore.adicionaVertice(v_fora_mst)
            arvore.adicionaAresta(arestaMenorPeso.getRotulo(), arestaMenorPeso.getV1(), arestaMenorPeso.getV2(), arestaMenorPeso.getPeso())
